...
const NotificationToast = ({ userId, onClose }: NotificationToastProps) => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        setLoading(true)
        const res = await Useraxios.get(`/notifications/${userId}`)
  ...
  const handleDeleteNotification = async (notificationId: string, e: React.MouseEvent) => {
    ...
    try {
      await Useraxios.delete(`/notifications/${notificationId}`)
  ...
  const handleMarkAsRead = async (notificationId: string, e: React.MouseEvent) => {
    e.stopPropagation() // prevent modal close
    try {
      await Useraxios.put(`/notifications/${notificationId}`, { isRead: true })
  ...
  return (
    // Wrapper: prevent clicks inside from closing modal
    <div

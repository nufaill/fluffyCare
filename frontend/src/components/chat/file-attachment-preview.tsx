          ...
          <div key={attachment.id} className="relative flex items-center gap-2 bg-muted rounded-lg p-2 max-w-xs">
            {attachment.type === 'image' && attachment.preview ? (
              <div className="relative">
                <img src={attachment.preview || '/placeholder.svg'} alt={attachment.file.name} className="h-12 w-12 object-cover rounded" />
                <Button
                  variant="destructive"
                  size="sm"
              ...
              <div className="flex items-center gap-2 flex-1">
                <div className="h-10 w-10 bg-primary/10 rounded-lg flex items-center justify-center">{getFileIcon(attachment.type)}</div>

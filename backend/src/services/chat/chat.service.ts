...
export class ChatService implements IChatService {
    ...
    async getShopChats(shopId: string, page: number = 1, limit: number = 20): Promise<ChatListResponseDTO> {
        try {
            this._validateObjectId(shopId, "Shop ID");
            this._validatePaginationParams(page, limit);
    ...
    async updateLastMessage(
        ...
        try {
            this._validateObjectId(chatId, "Chat ID");
            this._validateUpdateMessageData(updateData);
    ...
    private _validateUpdateMessageData(data: any): void {
    ...
    private _validateSearchDto(dto: ChatSearchDTO): void {

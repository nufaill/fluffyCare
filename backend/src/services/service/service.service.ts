...
export class ServiceService implements IServiceService {
  ...
  async createService(shopId: string, serviceData: CreateServiceDTO): Promise<ServiceDocument> {
    ...
    const nameExists = await this._serviceRepository.checkServiceNameExists(shopId, name);
    if (nameExists) {
      throw new CustomError('Service name already exists for this shop', 400);
    }

    const newService = await this._serviceRepository.createService({
      shopId,
      name: name.trim(),
  ...
  async getServiceByShopId(shopId: string): Promise<ServiceDocument[]> {
    return await this._serviceRepository.getServicesByShopId(shopId);
  }
  ...
  async updateService(serviceId: string, shopId: string, updateData: UpdateServiceDTO): Promise<ServiceDocument> {
    ...
    if (updateData.name && updateData.name.trim() !== existingService.name) {
      const nameExists = await this._serviceRepository.checkServiceNameExists(shopId, updateData.name, serviceId);
      if (nameExists) {
        throw new CustomError('Service name already exists for this shop', 400);
      }
    }

    const updatedService = await this._serviceRepository.updateService(serviceId, {
      ...updateData,
  ...
  async getAllServiceTypes(): Promise<ServiceTypeDocument[]> {
    return await this._serviceRepository.getAllServiceTypes();
  }

  async getAllPetTypes(): Promise<PetTypeDocument[]> {
    return await this._serviceRepository.getAllPetTypes();
  }
  ...
  async getAllServices(filters: ServiceFiltersDTO): Promise<{
    ...
    return await this._serviceRepository.getAllServices(filters);
  }
}